...
install-dlg-step4 = 4. Bingtray로 돌아가서 '재감지' 탭
install-dlg-open-shizuku = Shizuku 열기
install-dlg-detailed-instructions = 자세한 설명:
install-dlg-guide = 설치 가이드
install-dlg-guide-url = https://bingtray.pages.dev/docs/kr/docs/installation
...
# Main UI (bingtray.rs)
status-fetching-all = Bing 이미지 및 기록 데이터 가져오는 중...
status-fetching-images = Bing 이미지 가져오는 중...
status-downloading-history = 기록 데이터 다운로드 중...
status-loading-page = 더 많은 이미지 로드 중 (페이지 { $page })...
status-loading-high-res = 고해상도 이미지 로드 중...
image-title-label = 제목: { $title }
image-copyright-label = 저작권: { $copyright }
button-set-wallpaper = 이 배경화면 설정
button-set-cropped-wallpaper = 잘린 배경화면 설정
button-more-info = 추가 정보
dialog-loading-images = 이미지 로드 중
status-filter = 이미지 필터
status-all = 전체
status-keep-favorite = 즐겨찾기
