...
//! - [`get_next_historical_page()`] - Advance to next page and return page number (8 items/page)
...
//! - [`load_historical_images_paginated()`] - Load historical images with pagination (8 items/page)
//!
//! ### Metadata Search
...
use crate::{api_setwallpaper, duckdb_bingimage::*};
#[cfg(not(any(target_os = "android", target_arch = "wasm32")))]
...
pub fn load_historical_images_paginated(config: &Config, page: usize) -> Result<Vec<BingImage>> {
    ...
    // Load only 8 images for the current page using database pagination
    let limit = 8;
    let offset = page * 8;
