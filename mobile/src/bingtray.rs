//! Bingtray eframe UI (Cross-platform: Desktop, Android, WASM)
//!
...
use eframe::egui;
use egui_i18n::tr;
use egui::{Image, Pos2, pos2, Rect, Sense, Shape, Stroke, Vec2, emath};
use egui::epaint::StrokeKind;
use egui_material3::{menu, menu_item, top_app_bar};
...
pub struct BingtrayApp {
    ...
    #[cfg_attr(feature = "serde", serde(skip))]
    image_cache: HashMap<String, CarouselImage>,
    #[cfg_attr(feature = "serde", serde(skip))]
    ...
    #[cfg_attr(feature = "serde", serde(skip))]
    market_code_timestamps: HashMap<String, i64>,
    #[cfg_attr(feature = "serde", serde(skip))]
    ...
    #[cfg_attr(feature = "serde", serde(skip))]
    bing_loading_dialog_open: bool,
    #[cfg_attr(feature = "serde", serde(skip))]
    bing_loading_status: String,
    #[cfg_attr(feature = "serde", serde(skip))]
    bing_loading_progress: Option<Arc<std::sync::Mutex<String>>>,
    // Settings
    settings: Settings,
    // Dialog states
    #[cfg_attr(feature = "serde", serde(skip))]
...
impl Default for BingtrayApp {
    fn default() -> Self {
        ...
        // Market code loading functions removed, using default
        let (current_market_codes, market_code_timestamps) = {
            info!("Using default market codes (market code loading functions removed)");
            (vec!["en-US".to_string()], std::collections::HashMap::new())
        };
        info!("Final market codes: {:?}", current_market_codes);

        // Check for user mismatch (desktop user vs runtime user)
        let user_mismatch_warning = {
            let (current_user, desktop_user, is_different) = check_user_mismatch();
            if is_different {
                let warning = format!(
                    "Warning: Running as '{}' but desktop session is owned by '{}'. Wallpaper setting may not work.",
                    current_user, desktop_user
                );
                info!("{}", warning);
                Some(warning)
            } else {
                info!("User check: running as '{}', desktop user '{}' - OK", current_user, desktop_user);
                None
            }
        };
        ...
        Self {
            ...
            // Loading dialog state
            bing_loading_dialog_open: false,
            bing_loading_status: String::new(),
            bing_loading_progress: None,
            // Settings
...
impl BingtrayApp {
    fn ui(&mut self, ui: &mut egui::Ui) {
        ...
        self.show_navigation_menu(ui.ctx());

        // Show loading dialog
        self.show_bing_loading_dialog(ui.ctx());

        // Handle menu actions
        if MENU_SHOW_APP.swap(false, Ordering::Relaxed) {
            info!("Show App menu action - already showing app");
        }
        ...
        if !self.carousel_images.is_empty() {
            ...
            for (idx, carousel_img) in self.carousel_images.iter().enumerate() {
                ...
                carousel_widget = carousel_widget.item(Box::new(move |ui: &mut egui::Ui, _rect| {
                    ui.vertical_centered(|ui| {
                        ...
                        if response.clicked() {
                            info!("Carousel item {} clicked: {}", idx, title);
                            // Store clicked index in egui's persistent state
                            ui.ctx().data_mut(|d| {
            ...
            let _carousel_response = ui.add(carousel_widget);

            // Handle carousel click after rendering - check egui state
            let clicked_data = ui.ctx().data(|d| {
                let idx = d.get_temp::<usize>(egui::Id::new("carousel_clicked_index"));
                let url = d.get_temp::<String>(egui::Id::new("carousel_clicked_url"));
                (idx, url)
            });

            if let (Some(idx), Some(clicked_url)) = clicked_data {
                // Clear the clicked state immediately
                ui.ctx().data_mut(|d| {
                    d.remove::<usize>(egui::Id::new("carousel_clicked_index"));
                    d.remove::<String>(egui::Id::new("carousel_clicked_url"));
                });

                if let Some(carousel_image) = self.carousel_images.get(idx) {
                    if carousel_image.full_url == clicked_url {
                        ...
                        if let Some(cached_image) = self.image_cache.get(&carousel_image.full_url) {
                            ...
                            self.main_panel_image = Some(cached_image.clone());
            ...
            // Trigger when within 400px of the right edge
            ...
            match result {
                    ...
                    for bing_image in bing_images {
                            ...
                            let result = response.map(|response| {
                                ...
                                if response.status == 200 && !response.bytes.is_empty() {
                                    ...
                                    ctx.include_bytes(response.url.clone(), response.bytes.clone());
                                    ctx.request_repaint();
                                    let _image = Image::from_uri(response.url.clone());
                ...
                Err(e) => {
                    ...
                    if e.contains("No more historical data available") {
                        warn!("All historical data exhausted, disabling further infinite scroll attempts");
                        self.all_data_exhausted = true;
                    }
                    
                    // Show error message to user (but reduce frequency for "no more data" errors)
        ...
        if has_main_image {
            ...
            ui.horizontal(|ui| {
                if ui.button(tr!("button-set-wallpaper")).clicked() {
                    // Get data needed for wallpaper setting
                    let image_url = main_image.full_url.clone();
                    let image_title = main_image.title.clone();
                    let image_bytes = main_image.image_bytes.clone();
                    let setter = self.wallpaper_setter.clone();
                    let config = self.config.clone();

                    if image_bytes.is_none() {
                        error!("No image data available - image not yet loaded");
                        self.wallpaper_status = Some("✗ Image not loaded yet".to_string());
                        self.wallpaper_start_time = Some(SystemTime::now());
                    } else {
                        info!("Starting wallpaper setting: {}", image_title);

                        // Start wallpaper setting in background thread
                        std::thread::spawn(move || {
                            log::info!("BingtrayApp: Setting wallpaper in background thread");

                            // Save bytes to unprocessed directory (download only once!)
                            let bytes = image_bytes.unwrap();
                            let saved_path = match config {
                                Some(cfg) => {
                                    match crate::calc_bingimage::save_image_to_unprocessed(
                                        &cfg,
                                        &bytes,
                                        &image_url,
                                        &image_title,
                                        ...
                                        Ok(path) => Some(path),
                                        Err(e) => {
                                            log::error!("Failed to save image to unprocessed dir: {}", e);
                                            None
                                        }
                                    }
                                }
                            ...
                            let result = if let Some(path) = saved_path {
                                if let Some(setter) = setter {
                                    // Android: Use wallpaper setter with bytes
                                    #[cfg(target_os = "android")]
                                    {
                                        setter.set_wallpaper_from_bytes(&bytes)
                                    }
                                    #[cfg(not(target_os = "android"))]
                                    {
                                        let _ = setter;
                                        Err(std::io::Error::new(std::io::ErrorKind::NotFound, "No wallpaper setter"))
                                    }
                                } else {
                                    // Desktop: Set wallpaper from cached file
                                    #[cfg(not(any(target_os = "android", target_arch = "wasm32")))]
                                    {
                                        match crate::api_setwallpaper::set_wallpaper_from_cache(&path) {
                                            Ok(_) => Ok(true),
                                ...
                                Err(std::io::Error::new(std::io::ErrorKind::Other, "Failed to save image"))
                            };

                            match result {
                                Ok(true) => {
                                    log::info!("BingtrayApp: Wallpaper set successfully");
                                }
                                Ok(false) => {
                                    log::error!("BingtrayApp: Wallpaper setting failed");
                                }
                                Err(e) => {
                                    log::error!("BingtrayApp: Error setting wallpaper: {}", e);
                                }
                            }
                        });

                        // Update UI status
                        self.wallpaper_status = Some("✓ Wallpaper setting started".to_string());
                        self.wallpaper_start_time = Some(SystemTime::now());
                        ui.ctx().request_repaint_after(std::time::Duration::from_secs(1));
                        log::info!("BingtrayApp: Wallpaper setting request submitted");
                    }
                }
                ...
                if ui.button(tr!("button-set-cropped-wallpaper")).clicked() {
                    if let Some(bytes) = &main_image.image_bytes {
                        if !bytes.is_empty() {
                            ...
                            // Calculate crop rectangle using screen aspect ratio approach
                            ...
                            let crop_rect = {
                                ...
                                // Use selected rectangle coordinates, extending right edge to image right end
                                let (left, top, bottom, right) = if let Some(display_rect) = self.image_display_rect {
                                    ...
                                    let (actual_img_width, actual_img_height, _img_offset_x, _img_offset_y) = if image_aspect_ratio > display_aspect_ratio {
                                    ...
                                    // Convert square corners to relative coordinates
                                    ...
                                    let right = bitmap_width; // Always extend to image's right edge
                                    let bottom = ((rel_bottom * bitmap_height as f32).max(0.0)).min(bitmap_height as f32) as i32;

                                    // Ensure bottom is greater than top
                                    let bottom = if bottom <= top {
                                        warn!("Rectangle has zero or negative height! top={}, bottom={}, forcing minimum height", top, bottom);
                                        (top + bitmap_height / 4).min(bitmap_height)
                                    } else {
                                        bottom
                                    };

                                    info!("Final crop: left={}, top={}, right={}, bottom={}", left, top, right, bottom);
                                    (left, top, bottom, right)
                                } else {
                                    // Fallback if display rect is not available
                            ...
                            // Use Rust image cropping to pre-crop the bytes
                            ...
                            std::thread::spawn(move || {
                                ...
                                let result = if let Some(setter) = setter {
                                    // Android: Use injected wallpaper setter
                                    setter.set_wallpaper_from_bytes(&final_image_data)
                                } else {
                                    // Desktop: Use api_setwallpaper module directly
                                    #[cfg(not(any(target_os = "android", target_arch = "wasm32")))]
                                    {
                                        match crate::api_setwallpaper::set_wallpaper_from_bytes(&final_image_data) {
                                            Ok(_) => Ok(true),
                                            Err(e) => Err(std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))
                                        }
                                    }
                            ...
                            // Immediately update UI status
                            self.wallpaper_status = Some("✓ Cropped wallpaper setting started".to_string());
                            self.wallpaper_start_time = Some(SystemTime::now());
                            ui.ctx().request_repaint_after(std::time::Duration::from_secs(1));
                            log::info!("BingtrayApp: Finished processing cropped wallpaper setting request");
                        } else {
            ...
            if let Some(image_bytes) = &main_image.image_bytes {
                ...
                // Force consistent 16:9 aspect ratio
                ...
                if self.reset_rectangle_for_new_image {
                    ...
                    // Always update the screen ratio when image display changes
                    let (actual_screen_width, actual_screen_height) = self.get_actual_screen_size();
...
fn ui_mainpanel(
    ...
    tray_logic: &mut Option<BingTrayLogic>,
    menu_anchor_rect: &mut Option<Rect>,
) -> bool {
    let trigger_fetch = false;

    // TOP APP BAR
    let interactive_bar = top_app_bar(tr!("app-title"))
    ...
    *menu_anchor_rect = Some(Rect::from_min_size(
...
impl BingtrayApp {
    ...
    fn show_navigation_menu(&mut self, ctx: &egui::Context) {
        ...
        let next_market_item = if has_next_available {
            menu_item(&next_market_text)
                ...
                .on_click(|| {
                    MENU_NEXT_MARKET.store(true, Ordering::Relaxed);
                })
        } else {
            menu_item(&next_market_text)
                .leading_icon("skip_next")
        ...
        let keep_current_item = if can_keep {
            menu_item(&keep_text)
                .leading_icon("star")
                .on_click(|| {
                    MENU_KEEP_CURRENT.store(true, Ordering::Relaxed);
                })
        } else {
            menu_item(&keep_text)
                .leading_icon("star")
        ...
        let blacklist_current_item = if can_blacklist {
            menu_item(&blacklist_text)
                .leading_icon("block")
                .on_click(|| {
                    MENU_BLACKLIST_CURRENT.store(true, Ordering::Relaxed);
                })
        } else {
            menu_item(&blacklist_text)
                .leading_icon("block")
    ...
    fn show_bing_loading_dialog(&mut self, ctx: &egui::Context) {
        if self.bing_loading_dialog_open {
            use egui_material3::dialog;

            // Update status from shared progress if available
            if let Some(ref progress) = self.bing_loading_progress {
                if let Ok(status) = progress.lock() {
                    self.bing_loading_status = status.clone();
                }
            }

            dialog(
                "bing_loading_dialog",
                &tr!("dialog-loading-images"),
                &mut self.bing_loading_dialog_open,
            )
            .content(|ui| {
                ui.vertical_centered(|ui| {
                    ui.set_max_width(400.0);
                    ui.add_space(20.0);
                    ui.label(&self.bing_loading_status);
                    ui.add_space(10.0);
                    ui.spinner();
                    ui.add_space(20.0);
                });
    ...
    fn set_wallpaper_from_bytes_internal(&self, image_bytes: &[u8]) -> std::io::Result<bool> {
        if let Some(ref setter) = self.wallpaper_setter {
            ...
            #[cfg(not(any(target_os = "android", target_arch = "wasm32")))]
            {
                match crate::api_setwallpaper::set_wallpaper_from_bytes(image_bytes) {
                    Ok(_) => Ok(true),
                    Err(e) => Err(std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))
                }
            }
...
(1920.0, 1080.0) // Fallback for non-Android
    ...
    // Load market code timestamps from bingtray-core (removed - functions deleted)
    fn load_market_code_timestamps(&mut self) {
        info!("Market code loading functions removed - no-op");
    }
